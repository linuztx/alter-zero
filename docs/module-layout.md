...
| `layout.rs` | `live_height`, `repin` (plus `region_is_modal`/`repin_modal`, the covering re-pin a permission prompt gets — `docs/permissions.md`), `cursor_position`, `input_box` — the geometry policy `term.rs` acts on. |
