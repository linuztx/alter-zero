...
| `clipboard.rs` (+ `clipboard/linux.rs`) | Clipboard I/O — the boundary for **Ctrl+V image paste** (`read_clipboard_image`: clipboard image or copied image file → a kept temp PNG the backend reads by path — `docs/image-paste.md`; on Linux `linux.rs` streams the owner's own encoded bytes into that file before arboard is constructed — `image/png` then `jpeg`/`gif`/`webp`, a Wayland pipe or 1 MiB X11 property slices with `INCR` segments, capped at 256 MiB — so a paste decodes nothing — `docs/memory.md`) and **`/copy`** (`copy_to_clipboard`: arboard with an OSC 52 terminal-escape fallback for headless/SSH/tmux — `docs/copy.md`). The `base64`/OSC 52 framing is a tested pure core; the clipboard/filesystem I/O is smoke-covered like `term.rs`. | Pure parts: yes (I/O: smoke) |
