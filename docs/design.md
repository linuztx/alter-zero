...
- **Status indicator.** While a turn is in flight, the strip's status line opens
  ...
  **timer** in whole seconds, a cumulative **token** estimate (`↑` for uploaded
  ...
  `ALTER_ZERO_STARTUP_DELAY_MS`; its per-chunk pace, `CHUNK_DELAY`, is the
  twin knob `ALTER_ZERO_CHUNK_DELAY_MS`, which is how the offline backend
  mimics a model streaming a few tokens a second — `docs/slow-stream.md`) so
  the indicator is visibly working first: the
...
- **Clickable links (OSC 8).** Every URL an assistant reply shows — a bare
  ...
  wrap are unchanged; code and headings gain only the invisible target,
  detected across syntax-style boundaries before wrapping, and the dress is
  scoped to the link itself: a `[text](url)` suffix's decoration `(`/`)` keep
  the surrounding prose's colour rather than the target's blue + underline, so
  the run the eye reads as the link is the run a click opens. A terminal
  without OSC 8 consumes the escape and behaves exactly as before. The URL
  rides from the renderer (where it is still whole) to the paint boundary in
  ...
  **file** — `links::file_url`, the absolute `file://` URI behind whatever
  short form the row shows — so a click opens it, while the `⎿ Wrote …`
  corner row beneath stays plain. Gated by `ALTER_ZERO_HYPERLINKS` (default
  on); `smoke.sh` Phases 87 and 113.
...
- **`/model` picker** (`docs/llm.md`). An **inline** picker (it replaces the
  ...
  **persists the choice** to `~/.alter-zero/config.json` (`llm::settings::Settings`)
  ...
  it as its own (`docs/per-directory-state.md`) — and into the session's own
  rollout, so a resumed conversation comes back on the model it was on rather
  than the directory's current entry (`docs/session-model.md`). With no provider configured it shows a cyan
...
| `pty/`      | **Interactive terminal sessions** (`docs/interactive-shell.md`) — what `bash` with `tty` and `bash_session` run on: `keys` (the `<Enter>`/`<C-c>`/`<Up>` input notation, encoded to terminal bytes, and the double-escape undo), `transcript` (the output as lines — `vte`, carriage returns and erases applied), `screen` (the output as a screen — `vt100`, plus the terminal-query replies), `fold` (a plain command's piped output folded the same way, one line at a time — `\r` progress bars once, escapes gone, tabs kept), `probe` (what the session's processes are blocked in, from Linux's `/proc` — a read on the terminal, or a tree all at work), `settle` (when a waiting call returns: exit, a read the probe saw or a password prompt, prompt, line quiet, timeout), `report` (the `Running`/`Stopped` frames the model reads — `waiting for input`, or `waiting for a password` — and its model-only notes), `session` (`SessionIo` — the two views, the wait, the look, the exit handshake), and the boundary `spawn` (the pseudo-terminal, the controlling-terminal tiers, the line mode). The sessions themselves live in the background registry. | Pure cores: yes (`spawn`: real processes) |
...
| `images/` | **Inline pictures** (`docs/images.md`), split five ways: `geometry` (pure — the cell footprint a picture takes at the terminal's cell size under the **Image width** cap, reproducing `ratatui_image`'s own `Resize::Fit` arithmetic so the reservation and the encoder agree to the cell, plus the per-cell carrier that marks a block's rows and packs `(placement id, row)` into the top half of `links`' `underline_color` channel), `registry` (the process-global render policy and the `(path, cols, rows)` placement interner, so pure line builders can reserve a block without a new argument on every signature), `payload` (the boundary that downscales a picture for the **model** — `/settings` **Auto-resize images** — and keeps the result on disk for the session so a re-sent attachment is never decoded twice), `attachment` (the wire's copy of a picture — its base64 `data:` URL — encoded once per session, streamed from the file or the sidecar, and shared by reference by every request that re-sends it, so a turn allocates nothing picture-sized; `docs/memory.md`), `fitted` (pure — a PNG decoded at the size it will be shown or sent, its rows streamed through an area-average shrink so the whole picture is never held; `fit_box` reproduces the encoder's `Resize::Fit` arithmetic — `docs/memory.md`), and `store` (the paint boundary: the env + `TIOCGWINSZ` capability detection, the encoded pictures under a byte budget, and the one `stamp` pass every paint path runs). | Pure parts: yes (I/O: smoke, Phase 107; `tests/clipboard_linux.rs` under Xvfb) |
...
| `session.rs` | The **pure core of `/resume`** (`docs/resume.md`): the rollout JSONL format (`meta_line`/`item_line` on serde'd module-local record types — the app types stay serde-free), the forward-compatible `parse_session` (malformed/unknown lines skip), the two sidecars that ride the file without being history items — `checkpoint_line`/`parse_checkpoints` (`docs/checkpoint.md`) and `model_line`/`parse_model`, the session's own model selection whose newest record a resume runs on (`docs/session-model.md`) — the picker's `preview_of` (first user/shell message, flattened) and `relative_age` (codex's `now`/`Ns`/`Nm`/`Nh`/`Nd ago`), `rollout_rel_path` (the dated layout), and the `SessionMeta`/`SessionSummary` types. The recorder + dir scan are the boundary's (`tui::recorder::SessionRecorder`/`list_sessions`). | Yes |
...
| `project_doc.rs` | **AGENTS.md project-doc discovery** (`docs/project-doc.md`) — codex's `agents_md`: the pure `doc_chain` (project root → cwd, root first), `budget_docs` (the total byte budget, char-boundary truncation, blanks skipped — the docs kept separate, one `ProjectDoc { path, text }` each), `doc_budget` (the `ALTER_ZERO_PROJECT_DOC_MAX_BYTES` override; `0` disables), `doc_heading` (`Contents of {path} (project instructions, checked into the codebase):`, the override captioned `not checked in`) and `instructions_section` (the reference's override paragraph over one heading + text per file — the `<system-reminder>`'s first section, the codex `<INSTRUCTIONS>` fragment retired), over a thin fs probe (`find_project_root` — nearest `.git` dir *or* worktree gitfile; `load_user_instructions` — the first of `AGENTS.override.md`/`AGENTS.md` per directory, read **capped at the budget** and decoded lossily). `App::user_instructions` carries the section; `context::context_messages_full` wraps it into the block the derived context leads with. | Pure core: yes (fs probes: temp dirs) |
| `reminder.rs` | **The `<system-reminder>` the context leads with** (`docs/context.md`): `reminder_message` (the tags and the `Use the following contexts and instructions:` preamble over the non-blank sections, blank-line separated; empty when no section says anything, so a session with nothing to remind the model of sends no block) and `join_sections` (the shared join). The sections come from `project_doc::instructions_section`, `skills::skill_section` and `subagents::agent_section` (the latter two joined into `App::listings` by `subagents::listing_sections`); `context::context_messages_full` composes the block at derivation, `skills::listing_message` wraps the skills section alone as a subagent's briefing. | Pure core: yes |
| `term.rs`   | The custom inline viewport over `CrosstermBackend`: dynamic content-anchored height, `insert_before` (queues scrollback lines for the next frame — `docs/flicker.md`), `draw` (pending-flush + re-pin + diff + cursor, one synchronized update; the region is repainted **in place**, never blanked first, and only the rows the previous region left below it are cleared — `docs/slow-stream.md`), `reflow` (tail rebuild + live-region paint, one frame), the alternate-screen overlay (`enter_overlay`/`exit_overlay`/`draw_overlay`), init/restore (restore flushes leftovers), and the OSC 8 link emitter (`draw_cells` — the choke point every cell write goes through: strips the link-id carrier, brackets marked runs so a wrapped URL opens whole — `docs/links.md`). | No (I/O boundary) |
...
The real backend also retains one bounded request prefix (`llm::wire_history`)
to preserve provider tool IDs and parallel-call grouping across human turns.
It restores that prefix only after matching the rebuilt context; rewinds,
edits and incompatible histories keep the newly derived messages. See
[`prompt-caching.md`](prompt-caching.md) for its bounds and stress coverage.
Credential refreshes share a per-provider exchange gate, while login and
rotation share `EnvFile::update` for serialized, atomic credential-file updates.
...
streaming turn must show every live-region clear inside a sync block) — and,
since a commit repaints the region **in place** rather than blanking it first,
Phase 121 pins that a token-paced 65 s turn records no live-region clear at all
(`docs/slow-stream.md`).
...
- Interactive sessions (`bash` with `tty`) decide *waiting for input* first
  by asking Linux's `/proc` what the session's processes are blocked in (a
  `read` on the terminal is waiting, a tree all at work is busy), then by the
  terminal's own mode for a password prompt — a line read with echo off,
  which `sudo` shows even though its root process hides from `/proc`, named
  in the frame and never used to mask what is typed — and where the probe is
  blind — a process run as another user (`sudo` and what it runs), a
  `poll`-family wait, no `/proc` — by heuristics: the cursor left mid-line,
  the alternate screen, or a terminal read key by key (raw input with output
  processing still on, which a relay such as sudo's own pty does not leave),
  ...
  echoes `*` per key, or one behind a relay, is not named as one. See
