...
| `images/` | **Inline pictures** (`docs/images.md`), split three ways: `geometry` (pure — the cell footprint a picture takes at the terminal's cell size under the **Image width** cap, reproducing `ratatui_image`'s own `Resize::Fit` arithmetic so the reservation and the encoder agree to the cell, plus the per-cell carrier that marks a block's rows and packs `(placement id, row)` into the top half of `links`' `underline_color` channel), `registry` (the process-global render policy and the `(path, cols, rows)` placement interner, so pure line builders can reserve a block without a new argument on every signature), `payload` (the boundary that downscales a picture for the **model** — `/settings` **Auto-resize images**), and `store` (the paint boundary: the env + `TIOCGWINSZ` capability detection, the encoded pictures under a byte budget, and the one `stamp` pass every paint path runs). | Pure parts: yes (I/O: smoke, Phase 107) |
