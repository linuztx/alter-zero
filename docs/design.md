...
- Interactive sessions (`bash` with `tty`) decide *waiting for input* first
  ...
  `read` on the terminal is waiting, a tree all at work is busy), and where
  that is blind — a process run as another user (`sudo` and what it runs), a
  ...
  lists... `) can read as a prompt for a moment. See
