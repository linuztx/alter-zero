...
about the model (the `thinking` blob, `vision`, `context`; a fact still
unknown stays off the line, so a resume probes for it exactly as a launch on
a fact-less entry does). One format for "a saved model selection"
everywhere, so a future field reaches both files at once.
...
- on every `/model` pick;
- on every Ctrl+T cycle and on the startup capability probe's answer — the
  same two writers `config.json` has, so the facts are as fresh as the
  directory entry's would be;
...
recorded pair with the recorded thinking mode, vision and window, the Ctrl+T
cycle seeds where it left off, the footer and the context gauge follow —
...
- `scripts/smoke.sh` Phase 117 (the boundary): two instances in one
