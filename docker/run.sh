...
publish_spec() {
	...
	*)
	...
	# Fixed mappings need equal port counts, including a single port on one side.
	case "$host_port:$container_port" in
	:* | 0:*) ;; # Automatic host allocation can serve a container port range.
	*-*)
		...
		# Docker can choose one available host port from a range; Podman cannot.
		if [ "$engine" != docker ] || [ "$container_first" -ne "$container_last" ]; then
			[ "$((host_last - host_first))" -eq "$((container_last - container_first))" ] || fail "port ranges must have the same length: $1"
		fi
		;;
	esac
	if [ "$full_form" -eq 1 ]; then
		# Empty requests an available host port on both engines; Podman rejects 0.
		[ "$host_port" != 0 ] || host_port=""
		case "$address" in *:*) address="[$address]" ;; esac
		printf '%s:%s:%s%s\n' "$address" "$host_port" "$container_port" "$proto"
		return
	fi
	case "$bind" in
