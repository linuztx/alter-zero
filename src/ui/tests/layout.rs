        ...
        for &w in &widths {
            for &h in &heights {
                let run = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    ...
                    let _ = repaint_lines(&app.history, w, usize::from(h), &PathDisplay::VERBATIM);
...
fn size_sweep_apps() -> Vec<(&'static str, App)> {
    let base = || {
        ...
        app.start_tool("read_file", "src/app.rs with a long argument string", None);
    ...
    let tool = {
        ...
        app.start_tool("write_file", SWEEP_TEXT, None);
