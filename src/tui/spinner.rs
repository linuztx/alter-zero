...
//! `spinner.json` write (keyed by this working directory,
//! `docs/per-directory-state.md`) and the confirming toast — lives at the
//! boundary, the `tui::mascot::Session::select_mascot` pattern.
...
impl Session<'_> {
    /// Enter/Space in the `/spinner` picker: persist the choice — as this
    /// directory's own and the last made anywhere, a read-modify-write over
    /// the file (`config::save_look`) — and confirm it with a toast.
    ///
    ...
    pub(crate) fn select_spinner(&mut self, spinner: Spinner) {
        config::save_look(
            config::spinner_json_path().as_deref(),
