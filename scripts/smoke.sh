...
# at two heights — a region floating below a short conversation, and one flush
# at the bottom whose page FLOWS its top into scrollback (docs/view-flow.md).
# The screen AND the scrollback are compared byte for byte before and after. ---
po_round_trip() { # session, rows, cue, label
	local S90="$1" rows="$2" cue="$3" label="$4"
	...
	# Esc there closes the overlay; it must NOT arm the backtrack preview, which
	# would rewind history and prefill the composer the prompt has stashed.
	if ! printf '%s' "$in_o" | grep -qF "q/esc/ctrl+o to quit"; then
		echo "FAIL: Phase 90 ($label) — the overlay offers Esc as a backtrack while a prompt waits" >&2
		status=1
	fi
