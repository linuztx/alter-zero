...
# arrives) instead of the band growing. /quit is the 10th of ten commands
# (docs/settings.md added /settings as the 9th), so nine presses reach it.
for _ in $(seq 1 9); do
...
# the open palette) — the first 8 only: the 9th (/settings) and 10th (/quit)
# start off-window and ↓ scrolls them in.
...
if printf '%s' "$palette_open" | grep -qF "Exit alter-zero"; then
	echo "FAIL: the palette shows /quit (the 10th command) in its first window — the 8-row cap is gone" >&2
	status=1
fi
...
if ! printf '%s' "$perm_prompt" | grep -qF "2. Yes, allow all edits during this session (ctrl+a)"; then
...
# --- Phase 64: auto + master permission modes (docs/permissions.md). Ctrl+A
...
# One more Ctrl+A step: auto → master.
tmux send-keys -t "$S64" C-a
sleep 0.3
...
if ! printf '%s' "$auto_footer" | grep -qE "auto\s*$" ||
	...
	echo "FAIL: Phase 64 — two Ctrl+A steps did not land the footer/toast on auto" >&2
	status=1
fi
if [ -z "$auto_done" ]; then
...
if ! printf '%s' "$master_footer" | grep -qE "master\s*$"; then
	echo "FAIL: Phase 64 — the third Ctrl+A step did not land the footer on master" >&2
	status=1
fi
...
# "User answered Claude's questions:" cell; the composer draft typed before
...
for _ in $(seq 1 250); do
	cap="$(tmux capture-pane -t "$S68" -p)"
	if printf '%s' "$cap" | grep -qF "User answered Claude's questions:" &&
...
# three that made alter0 "take seconds to boot": a SHARED scratch parent
