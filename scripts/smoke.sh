...
# question about something visible, not a box out of nowhere. Header only: it
# is waiting on the user, not on a queue.
...
if printf '%s' "$perm_prompt" | grep -qF "Waiting…"; then
	echo "FAIL: Phase 55 — the call under the prompt still shows a ⎿ Waiting… row" >&2
	status=1
fi
