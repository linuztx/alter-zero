...
# paragraph under the banner that shifts every row assertion, and a real ping
# per phase. Off for the whole suite, both exported (the tmux server inherits
# it) and spelled out in every launch string (a pre-existing tmux server does
# not); Phase 115 unsets it and points the app at a local stub instead.
...
cleanup() {
	...
	tmux kill-session -t "${S}_filelink" 2>/dev/null
	...
	tmux kill-session -t "${S}_agentgauge" 2>/dev/null
	...
	tmux kill-session -t "${S}_telemetry" 2>/dev/null
	[ -n "${TM_SERVER:-}" ] && kill "$TM_SERVER" 2>/dev/null
	[ -n "${TM_CFG:-}" ] && rm -rf "$TM_CFG" 2>/dev/null
...
for _ in $(seq 1 200); do # the background launch puts its row on the roster
	if tmux capture-pane -t "$S111" -p | grep -qF "Stream a comparison table"; then
		break
	fi
	sleep 0.1
done
...
printf '%s\n' "$gauge_fresh"
if ! printf '%s' "$gauge_fresh" | grep -qE '(^|[^0-9.k])0/100k \(0\.0%\)'; then
	echo "FAIL: Phase 111 — a just-opened agent view's gauge is not the agent's true zero (the lead's count leaked in): '$gauge_fresh'" >&2
	status=1
fi
# The agent's round settles: the table closes and its receipt lands.
for _ in $(seq 1 300); do
	if tmux capture-pane -t "$S111" -p -S -200 | grep -qF "Four rows, one grid."; then
		break
	fi
	sleep 0.1
done
...
if [ -z "$agent_used" ] || [ "$agent_used" = "0/100k" ]; then
	echo "FAIL: Phase 111 — the settled agent's gauge shows no estimate of its own transcript: '$gauge_agent'" >&2
	status=1
fi
...
if [ -z "$main_used" ] || [ "$main_used" = "0/100k" ]; then
	echo "FAIL: Phase 111 — the main footer lost its gauge after leaving the agent view: '$gauge_main'" >&2
	status=1
fi
if [ -n "$agent_used" ] && [ "$agent_used" = "$main_used" ]; then
	echo "FAIL: Phase 111 — the agent view's gauge ($agent_used) is the main session's ($main_used): the footer is not following the conversation on screen" >&2
	status=1
fi
...
# --- Phase 111: the `/theme` picker (docs/theme.md). The `/spinner` picker's
...
if ! printf '%s\n' "$theme_banner_raw" | grep -F "/login" | grep -qF "$THEME_MOCHA_SGR"; then
	echo "FAIL: Phase 111 — the banner's /login hint is not in Mocha's sky accent at launch" >&2
...
printf '%s\n' "$theme_palette"
if ! printf '%s' "$theme_palette" | grep -qF "Choose the colour theme"; then
	echo "FAIL: Phase 111 — /theme is missing from the slash-command palette" >&2
	status=1
fi
tmux send-keys -t "$S111" Enter
sleep 0.5
...
for expect in "→ mocha" "macchiato" "frappe" "latte" "onedark" "dracula" "nord" "gruvbox" "(1/11)" \
	...
	"Type to search · Enter to choose · Esc to cancel"; do
	if ! printf '%s' "$theme_open" | grep -qF -- "$expect"; then
		echo "FAIL: Phase 111 — the open picker is missing '$expect'" >&2
		status=1
	fi
done
# Every listed row wears its five-swatch palette sample.
if [ "$(printf '%s\n' "$theme_open" | grep -cE '^ *(→ )?[a-z]+ +●●●●●')" -lt 10 ]; then
	echo "FAIL: Phase 111 — the theme rows do not wear their swatches" >&2
	status=1
fi
...
if [ "$(printf '%s\n' "$theme_open" | grep -c '✓')" -ne 1 ] || ! printf '%s\n' "$theme_open" | grep -F "→ mocha" | grep -qF "✓"; then
	echo "FAIL: Phase 111 — the ✓ does not sit on the mocha row alone" >&2
	status=1
fi
...
printf '%s\n' "$theme_down"
if ! printf '%s' "$theme_down" | grep -qF "(2/11)"; then
	echo "FAIL: Phase 111 — the counter did not follow the selection" >&2
	status=1
fi
if ! printf '%s' "$theme_down" | grep -qF "Catppuccin Macchiato — the medium-dark flavour"; then
	echo "FAIL: Phase 111 — the description did not follow the selection" >&2
	status=1
fi
...
if ! printf '%s' "$theme_filtered" | grep -qF "(1/1)"; then
	echo "FAIL: Phase 111 — 'drac' did not narrow the list to dracula" >&2
	status=1
fi
tmux send-keys -t "$S111" Enter
sleep 0.8
...
printf '%s\n' "$theme_after"
if ! printf '%s' "$theme_after" | grep -qF "Theme: dracula"; then
	echo "FAIL: Phase 111 — the switch was not confirmed with a toast" >&2
	status=1
fi
if ! grep -qF '"theme": "dracula"' "$THEME_CFG/theme.json" 2>/dev/null; then
	echo "FAIL: Phase 111 — theme.json was not written (or holds the wrong theme)" >&2
	status=1
fi
...
if ! printf '%s\n' "$theme_after_raw" | grep -F "/login" | grep -qF "$THEME_DRACULA_SGR"; then
	echo "FAIL: Phase 111 — the banner hint was not recoloured into Dracula's accent" >&2
	printf '%s\n' "$theme_after_raw" | grep -F "/login" | cat -v >&2
	status=1
fi
if printf '%s\n' "$theme_after_raw" | grep -F "/login" | grep -qF "$THEME_MOCHA_SGR"; then
	echo "FAIL: Phase 111 — the banner hint still carries Mocha's accent after the switch" >&2
	status=1
fi
if ! printf '%s' "$theme_after" | grep -qF "dummy_model_name ·"; then
	echo "FAIL: Phase 111 — the composer and footer did not come back after the switch" >&2
	status=1
fi
tmux send-keys -t "$S111" -l "/quit"
sleep 0.2
tmux send-keys -t "$S111" Enter
sleep 0.6
tmux kill-session -t "$S111" 2>/dev/null
...
if ! printf '%s\n' "$theme_relaunch_raw" | grep -F "/login" | grep -qF "$THEME_DRACULA_SGR"; then
	echo "FAIL: Phase 111 — a fresh launch did not paint the banner in the saved theme" >&2
...
printf '%s\n' "$theme_relaunch"
if ! printf '%s' "$theme_relaunch" | grep -qF "(6/11)"; then
	echo "FAIL: Phase 111 — the saved theme did not seat the highlight on dracula after a relaunch" >&2
	status=1
fi
if ! printf '%s\n' "$theme_relaunch" | grep -F "→ dracula" | grep -qF "✓"; then
	echo "FAIL: Phase 111 — the saved theme does not wear the ✓ after a relaunch" >&2
	status=1
fi
tmux kill-session -t "$S111" 2>/dev/null
rm -rf "$THEME_CFG"
...
# --- Phase 113: the FILE TOOL HEADER'S PATH IS A file:// LINK (docs/links.md
...
S111="${S}_filelink"
FILELINK_RAW="$(mktemp /tmp/alter-zero-smoke-filelink-XXXXXX)"
tmux new-session -d -s "$S111" -x 80 -y 40 "$APP"
tmux pipe-pane -t "$S111" -o "cat >> $FILELINK_RAW"
sleep 0.4
tmux send-keys -t "$S111" -l "$USER_MSG"
sleep 0.2
tmux send-keys -t "$S111" Enter
for _ in $(seq 1 150); do
	if tmux capture-pane -t "$S111" -p -S -200 | grep -qF "$SETTLED_REPLY"; then
		break
	fi
	sleep 0.1
done
...
for expect in "● Read(about.py)" "● Edit(about.py)"; do
	if ! printf '%s' "$filelink_pane" | grep -qF "$expect"; then
		echo "FAIL: Phase 113 — '$expect' is missing from the pane" >&2
		status=1
	fi
done
...
*[!A-Za-z0-9/._~-]*)
	if ! grep -aqE $']8;id=az[0-9]+;file:///[^\x1b]*/about\\.py\x1b' "$FILELINK_RAW"; then
		echo "FAIL: Phase 113 — no OSC 8 open carries a file:// URI ending in /about.py" >&2
		status=1
	fi
	;;
*)
	if ! grep -aqF ";${filelink_url}"$'\x1b' "$FILELINK_RAW"; then
		echo "FAIL: Phase 113 — no OSC 8 open carries ${filelink_url} in the raw stream" >&2
		status=1
	fi
	;;
esac
if ! grep -aqF $'\x1b]8;;\x1b' "$FILELINK_RAW"; then
	echo "FAIL: Phase 113 — the OSC 8 close is missing from the raw stream" >&2
	status=1
fi
if grep -aqF $'\x1b[58;' "$FILELINK_RAW"; then
	echo "FAIL: Phase 113 — the link-id carrier leaked as an SGR 58 underline colour" >&2
	status=1
fi
...
if ! printf '%s' "$filelink_runs" | grep -qF "about.py"; then
	echo "FAIL: Phase 113 — the path text is not what the file:// link wraps" >&2
	status=1
fi
if printf '%s' "$filelink_runs" | grep -qE '⎿|Read [0-9]+ lines|Updated'; then
	echo "FAIL: Phase 113 — a corner row rode inside the file:// link" >&2
	status=1
fi
tmux kill-session -t "$S111" 2>/dev/null
rm -f "$FILELINK_RAW"
...
# --- Phase 114: PER-DIRECTORY LOOKS (docs/per-directory-state.md). /mascot and
...
S114="${S}_looks"
LK_CFG="$(mktemp -d)"
LK_A="$(mktemp -d /tmp/alter-zero-smoke-looks-a-XXXXXX)"
LK_B="$(mktemp -d /tmp/alter-zero-smoke-looks-b-XXXXXX)"
LK_C="$(mktemp -d /tmp/alter-zero-smoke-looks-c-XXXXXX)"
# The paths as the app keys them — the kernel's cwd, symlinks resolved.
...
lk_expect() { # file, what, expected, actual
	if [ "$4" != "$3" ]; then
		...
		cat "$1" 2>/dev/null
		echo "FAIL: Phase 114 — $2: expected '$3', got '$4'" >&2
		status=1
	fi
}
...
printf '%s\n' "$lk_a_after"
if ! printf '%s' "$lk_a_after" | grep -qF "▀█▄███▄█▀"; then
	echo "FAIL: Phase 114 — the banner did not redraw with bloom" >&2
	status=1
fi
...
printf '%s\n' "$lk_b_boot"
if ! printf '%s' "$lk_b_boot" | grep -qF "▀█▄███▄█▀"; then
	echo "FAIL: Phase 114 — a new directory did not start from the last mascot (bloom)" >&2
	status=1
fi
...
if ! printf '%s\n' "$lk_b_spin" | grep -F "→ dots" | grep -qF "✓"; then
	echo "FAIL: Phase 114 — a new directory did not start from the last spinner (dots)" >&2
	status=1
fi
...
printf '%s\n' "$lk_a_again"
if ! printf '%s' "$lk_a_again" | grep -qF "▀█▄███▄█▀"; then
	echo "FAIL: Phase 114 — the first directory lost its own mascot (bloom)" >&2
	status=1
fi
if printf '%s' "$lk_a_again" | grep -qF "▝▀██▄██▀▘"; then
	echo "FAIL: Phase 114 — the first directory followed the second's mascot (gem)" >&2
	status=1
fi
tmux send-keys -t "$S114" -l "/spinner"
sleep 0.3
...
if ! printf '%s\n' "$lk_a_spin" | grep -F "→ dots" | grep -qF "✓"; then
	echo "FAIL: Phase 114 — the first directory lost its own spinner (dots)" >&2
	status=1
fi
tmux send-keys -t "$S114" Escape
sleep 0.3
...
printf '%s\n' "$lk_c_boot"
if ! printf '%s' "$lk_c_boot" | grep -qF "▝▀██▄██▀▘"; then
	echo "FAIL: Phase 114 — a third directory did not start from the new last mascot (gem)" >&2
	status=1
fi
lk_quit "$S114"
...
# --- Phase 115: telemetry — one anonymous ping a day, disclosed once, off with one setting (docs/telemetry.md) ---
...
S115="${S}_telemetry"
TM_CFG="$(mktemp -d /tmp/alter-zero-smoke-telemetry-XXXXXX)"
