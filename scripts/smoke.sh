...
if [ -z "$tasks_done" ]; then
	...
	if printf '%s' "$tasks_done" | grep -qF "◻ Set up the project structure"; then
		echo "FAIL: Phase 69 — the checklist leaked into the idle screen (it is a turn-time display)" >&2
		status=1
	fi
fi
