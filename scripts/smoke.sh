...
# arrives) instead of the band growing. /quit is the 11th of eleven commands
# (docs/settings.md added /settings as the 9th, docs/hooks-menu.md /hooks as
# the 10th), so ten presses reach it.
for _ in $(seq 1 10); do
...
# the open palette) — the first 8 only: the 9th (/settings), 10th (/hooks) and
# 11th (/quit) start off-window and ↓ scrolls them in.
