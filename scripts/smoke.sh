...
# --- Phase 38: PARALLEL tool-call batch — a not-yet-run call shows `⎿ Waiting…`
# (docs/parallel-tools.md). The dummy announces a two-call batch (a Read then a
# Bash) up front: while the `● Read(src/main.rs)` runs, the `● Bash(…)` sibling
# shows a dim `⎿ Waiting…` in the live region above the box, both cells visible at
# once. Drive a fresh, tall session and poll for that transient state (the batch
# runs after the pre-stream pause + the first text + thinking). ---
S_BATCH="${S}_batch"
tmux new-session -d -s "$S_BATCH" -x 80 -y 40 "$APP"
sleep 0.4
tmux send-keys -t "$S_BATCH" -l "look into the code"
sleep 0.2
tmux send-keys -t "$S_BATCH" Enter
batch_waiting=""
for _ in $(seq 1 120); do # up to ~18s (pause + first-half text + thinking, then tools)
...
echo "==== Phase 38: captured pane (parallel batch — a running Read cell + a ⎿ Waiting… Bash sibling) ===="
printf '%s\n' "$batch_waiting"
