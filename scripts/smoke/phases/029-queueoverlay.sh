...
# summary.
overlay_advanced="$(wait_pane 45 "$S29" -F "Done for")" # up to ~45s: the turn reads it, then finishes
...
expect_has "$overlay_advanced" -F "Done for" "the turn that read the queued message never ran to its 'Done for' summary under the overlay"
...
expect_has "$queue_overlay_returned" -F "Done for" "after returning from the overlay the turn's summary is missing from the inline view"
