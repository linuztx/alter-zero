...
queueint="$(wait_pane 20.1 "$S10" -S -60 -- -F "$SUMMARY_TURN2")" # up to ~20s: the flushed "world" turn finishes
...
expect_has "$queueint" -F "$SUMMARY_TURN2" "the queued message sent on interrupt never finished its turn"
