#!/usr/bin/env bash
# Phase 117 — PER-SESSION model: a conversation resumes on the model it ran on
...
launch -c "$SM_DIR" "$S117" 100 30 "$APP_SM"
sm_a="$(wait_pane 5 "$S117" -F "model-a")"
...
submit "$S117" "!echo first"
sm_a_turn="$(wait_pane 10 "$S117" -E "⎿ +first")"
...
launch -c "$SM_DIR" "$S117" 100 30 "env ALTER_ZERO_MODEL=model-b $APP_SM"
sm_b="$(wait_pane 5 "$S117" -F "model-b")"
...
submit "$S117" "!echo second"
sm_b_turn="$(wait_pane 10 "$S117" -E "⎿ +second")"
...
sm_d="$(wait_pane 5 "$S117" -F "model-a")"
...
launch -c "$SM_DIR" "$S117" 100 30 "$APP_SM --continue"
sm_e="$(wait_pane 5 "$S117" -F "model-b")"
...
launch -c "$SM_DIR" "$S117" 100 30 "$APP_SM"
sm_f="$(wait_pane 5 "$S117" -F "model-c")"
...
tmux send-keys -t "$S117" -l "/resume"
sleep 0.3
tmux send-keys -t "$S117" Enter
sm_picker="$(wait_pane 5 "$S117" -F "R E S U M E")"
expect_has "$sm_picker" -F "R E S U M E" "/resume did not open the session picker"
tmux send-keys -t "$S117" -l "first"
sleep 0.4
tmux send-keys -t "$S117" Enter
sm_f2="$(wait_pane 5 "$S117" -F "model-a")"
...
launch -c "$SM_DIR" "$S117" 100 30 "env ALTER_ZERO_MODEL=model-p $APP_SM --resume $SM_F1"
sm_g="$(wait_pane 5 "$S117" -F "model-p")"
...
launch -c "$SM_DIR" "$S117" 100 30 "$APP_SM --resume $SM_F3"
sm_h="$(wait_pane 4 "$S117" -F "Can't resume on ghost")"
