#!/usr/bin/env bash
# Phase 123 — Esc mid-batch keeps a red cell for EVERY call (running and waiting) and the context replays them all
...
# the next request derives: all three calls, each answered.
S_INT="${S}_batchint"
...
note "the pane after Esc mid-batch"
printf '%s\n' "$interrupted"
...
# the screen from history, which is why that one used to show).
S_PERM="${S}_batchperm"
launch "$S_PERM" 90 40
