...
overlay_done="$(wait_pane 20.1 "$S4" -F "Done for")" # up to ~20s
...
if ! printf '%s' "$overlay_done" | grep -qF "Done for"; then
	...
	expect_has "$post_quit" -F "Done for" "after quitting (Ctrl+C) from the overlay, the committed 'Done for Ns' summary was not restored to the screen — the quit path left the stale live status strip behind"
