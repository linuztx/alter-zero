...
# an interactive session's cells (docs/bash-tools.md). The dummy's
# `interactive` demo plays the shape a live model produces: `bash` stopping
# at a prompt, then `bashsend` answering one question a round until the
# program exits — every result the real report formatter's. What only the
# real binary shows is how those reports land on screen: the frame line
# (`Running (session …, waiting for input)`) is the MODEL's, so it must never
# reach a cell; a session still at its prompt closes its cell on a dim
# `⎿ Waiting for input · session …` row instead; an answer's header names the
# command it types into and what was typed (`BashSend(./configure.sh ←
...
printf '%s\n' "$interactive"
expect_has "$interactive" -F "● Bash(./configure.sh)" "the launch cell is missing"
...
expect_has "$interactive" -F "● BashSend(./configure.sh ← demo⏎)" "the answer cell does not name the command and the input"
expect_has "$interactive" -F "Install into ./demo? [Y/n]" "the answer cell does not show the next question"
expect_has "$interactive" -F "● BashSend(./configure.sh ← y⏎)" "the last answer's cell is missing"
