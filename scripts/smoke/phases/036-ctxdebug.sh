...
wait_for 20 "$S36" -E "^$SUMMARY_TURN1 [0-9]+s" # let the turn finish so the tools are in history
...
expect_has "$ctxdebug_returned" -F "$SUMMARY_TURN1" "the conversation did not repaint after closing the context-debug view"
