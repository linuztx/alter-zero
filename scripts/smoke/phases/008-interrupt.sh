...
# strip clears (no "tokens" line), and NO turn summary appears. The app
# keeps running — a follow-up message must stream and finish normally
# ($SUMMARY_TURN2, turn 2's summary). Esc when idle now arms the Esc-Esc
...
after_interrupt="$(wait_pane 20.1 "$S5" -S -30 -- -F "$SUMMARY_TURN2")" # up to ~20s: wait for the follow-up turn's summary
...
expect_lacks "$interrupted" -E "$SUMMARY_ANY_RE" "an interrupted turn must not commit a summary (the notice is its terminal state)"
expect_has "$after_interrupt" -F "$SUMMARY_TURN2" "the app did not complete a follow-up turn after the interrupt — the loop or backend channel is wedged"
