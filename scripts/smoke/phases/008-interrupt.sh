...
after_interrupt="$(wait_pane 20.1 "$S5" -S -30 -- -F "Finished for")" # up to ~20s: wait for the follow-up turn's summary
...
expect_lacks "$interrupted" -F "Done for" "an interrupted turn must not commit a 'Done for Ns' summary (the notice is its terminal state)"
expect_has "$after_interrupt" -F "Finished for" "the app did not complete a follow-up turn after the interrupt — the loop or backend channel is wedged"
