...
offline against real archives (docs/release.md).
    ...
    python3 scripts/release/release_server.py DIST TAG

Serves DIST's files at /releases/download/TAG/<name>, redirects
/releases/latest to /releases/tag/TAG the way GitHub does (the installer
reads the tag off that redirect), and answers /releases/tag/TAG with a stub
page. Binds an ephemeral port on 127.0.0.1 and prints it on the first line
of stdout; the selftest points ALTER_ZERO_INSTALL_BASE_URL at it.
"""
import http.server
import os
import socketserver
